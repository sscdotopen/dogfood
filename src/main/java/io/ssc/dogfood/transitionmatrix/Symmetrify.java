...
public class Symmetrify implements PlanAssembler {
  ...
  public Plan getPlan(String... args) {
    ...
    ReduceContract reducer = new ReduceContract.Builder(ToAdjacencyListReducer.class, PactLong.class, 0)
        .input(symmetryMap)
        .name("ToRowsReducer")
        .build();
    ...
    RecordOutputFormat.configureRecordFormat(out)
        ...
        .field(PactLongArray.class, 1);

    Plan plan = new Plan(out, "Symmetrify");
  ...
  public static class ToAdjacencyListReducer extends ReduceStub {
    ...
    public void reduce(Iterator<PactRecord> records, Collector<PactRecord> collector) throws Exception {
      ...
      neighbors.add(first.getField(1, PactLong.class).getValue());

      while (records.hasNext()) {
        long neighborId = records.next().getField(1, PactLong.class).getValue();
      ...
      record.setField(1, new PactLongArray(neighbors));
